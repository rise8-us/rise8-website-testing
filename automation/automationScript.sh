...
function checkScore() {
  ...
  if (( $(echo "$1 < $threshold" | bc -l) )); then
      ...
      curl -X POST -H 'Content-type: application/json' --data "{\"text\":\"$2 score is below 90! $2 score: $1\"}" "$SLACK_SECRET"
  else
      echo "$2 score is above or equal to 90. Current score: $1"
      curl -X POST -H 'Content-type: application/json' --data "{\"text\":\"$2 score is above or equal to 90. $2 score: $1\"}" "${SLACK_SECRET}"
  fi
}
